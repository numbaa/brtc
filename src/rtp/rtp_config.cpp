...
uint32_t FindAssociatedSsrc(uint32_t ssrc,
  ...
  assert(ssrcs.size() == associated_ssrcs.size());
...
std::string RtpConfig::ToString() const {
  ...
  ss << "], mid: '" << mid << "'";
  ss << ", rtcp_mode: "
     << (rtcp_mode == brtc::RtcpMode::kCompound ? "RtcpMode::kCompound"
...
bool RtpConfig::IsMediaSsrc(uint32_t ssrc) const {
  return std::ranges::find(ssrcs, ssrc) != ssrcs.cend();
}

bool RtpConfig::IsRtxSsrc(uint32_t ssrc) const {
  return std::ranges::find(rtx.ssrcs, ssrc) != rtx.ssrcs.cend();
}
...
uint32_t RtpConfig::GetMediaSsrcAssociatedWithFlexfecSsrc(
  ...
  assert(IsFlexfecSsrc(flexfec_ssrc));
  ...
  // SSRC.
  assert(1u == flexfec.protected_media_ssrcs.size());
