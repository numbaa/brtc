...
#include "video/depacketizer/bit_buffer.h"
#include <cassert>
#include <algorithm>
#include <limits>
...
BitBuffer::BitBuffer(const uint8_t* bytes, size_t byte_count)
  ...
  assert(static_cast<uint64_t>(byte_count_) <=
...
bool BitBuffer::ReadUInt8(uint8_t* val) {
  ...
  assert(bit_val <= std::numeric_limits<uint8_t>::max());
...
bool BitBuffer::ReadUInt16(uint16_t* val) {
  ...
  assert(bit_val <= std::numeric_limits<uint16_t>::max());
...
bool BitBuffer::ReadNonSymmetric(uint32_t* val, uint32_t num_values) {
  assert(num_values > 0);
  assert(num_values <= uint32_t{1} << 31);
  if (num_values == 1) {
...
bool BitBuffer::ReadExponentialGolomb(uint32_t* val) {
  ...
  assert(!PeekBits(&peeked_bit, 1) || peeked_bit == 1);
  ...
  if (value_bit_count > 32 || !ReadBits(val, value_bit_count)) {
    assert(Seek(original_byte_offset, original_bit_offset));
    return false;
  }
  *val -= 1;
  return true;
